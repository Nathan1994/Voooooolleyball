...
static const int BALL_TAG = 0x001;
static const int PLAYER_TAG = 0x111;
static const int WALL_TAG = 0x011;
static const int GROUND_TAG = 0x100;
static const int ENEMY_TAG = 0x101;
static const int OBSTACLE_TAG = 0x110;
...
void GamePlayerLayer::configureObstacle(){
    ...
    highObstacle->setPhysicsBody(highObstacleBody);
    this->addChild(highObstacle);
...
//    player->setPosition(Point(VisibleRect::getVisibleRect().size.width/5,VisibleRect::getVisibleRect().size.height*(1-offsetScale)+1));
    ...
    player->getPhysicsBody()->setCategoryBitmask(PLAYER_TAG);
    player->getPhysicsBody()->setCollisionBitmask(PLAYER_TAG);
...
void GamePlayerLayer::configureEnemy(){
    ...
    enemy->getPhysicsBody()->setCategoryBitmask(ENEMY_TAG);
    enemy->getPhysicsBody()->setCollisionBitmask(ENEMY_TAG);
...
void GamePlayerLayer::configureEdge(){
    ...
    groundBody->setCategoryBitmask(WALL_TAG);
    groundBody->setCollisionBitmask(WALL_TAG);
...
void GamePlayerLayer::hitBall(){
    ...
    if ((VisibleRect::center().x - ball->getPosition().x > 0)) {
